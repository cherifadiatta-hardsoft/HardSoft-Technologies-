...
files.forEach(file => {
  ...
  // We need to avoid double replacing. 
  for (const [key, value] of Object.entries(replacements)) {
    // Only replace if neither value nor dark:var exists 
    // It's a bit tricky. We will regex replace whole words.
    // If the word has `dark:` right before it, or if `dark:` + key is already there, don't replace
    
    // Instead of complex lookarounds, since we haven't done it yet, we just replace all keys 
    // EXCEPT if value is already there, or if 'dark:' + key is in the file.
